      ...
      outputFun(value1) {
        ...
        if (value1.length < 10) {
          errorCheck.innerHTML =
            "<li>string needs to be at least 10 characters long</li>";
          const result = errorCheck.innerHTML;
          errorArr.push(result);
        }
        if (!/[A-Z]/.test(value1)) {
          errorCheck.innerHTML =
            "<li>string needs to include at least one uppercase character</li>";
          const result = errorCheck.innerHTML;
          errorArr.push(result);
        }
        if (!/[a-z]/.test(value1)) {
          errorCheck.innerHTML =
            "<li>string needs to include at least on lowercase character</li>";
          const result = errorCheck.innerHTML;
          errorArr.push(result);
        }
        if (!/\d/.test(value1)) {
          errorCheck.innerHTML =
            "<li>string needs to include at least one number</li>";
          const result = errorCheck.innerHTML;
          errorArr.push(result);
        } else {
          return true;
  ...
  calculationFunctions.forEach((res) => {
    ...
    if (
      ...
      submit.addEventListener("click", () => {
        ...
        if (value1 && value2) {
          if (
            (typeof value1.value === "string", typeof value1.value === "string")
          ) {
            const num1 = value1.value;
            const num2 = value2.value;
          ...
          if (
            Number.isInteger(value1.value) &&
            Number.isInteger(value2.value)
          ) {
